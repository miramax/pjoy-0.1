...
abstract class Controllers {
  ...
  protected function render( $tpl = null, $args=null ) {
    $templater = new AppTemplate();

    $cls = new ReflectionClass($this);
    $controller = $cls->getName();
    $pathName = strtolower(str_replace('Controller', '', $controller));
    $path = ROOT . DS . 'www' . DS . 'app' . DS . 'views' . DS;
    $templater->setViewPath($path . $pathName . DS);

    if(defined('BACKEND')){
      $fname = 'back'.$pathName .'_'. $tpl;
    } else {
      $fname = 'front'.$pathName .'_'. $tpl;
    }

    $fname = str_replace(array('/', '..'), array('', ''), $fname);
    $tpl = !strstr($tpl, '.p')? $tpl . '.phtml' : $tpl;
  ...
  protected function display($template, $data = null) {
    $this->content = $this->render($template.'.phtml', $data);
  }
  ...
  protected static function Component($name,$args=null) {
    $cls = $name.'Component';
    $component = new $cls();
    if(!$args)
